...
export async function updateTenantSettings(
  ...
  for (const d of [0, 1, 2, 3, 4, 5, 6]) {
    ...
    const hasLunchBreak = formData.get(`hasLunchBreak-${d}`) === "on";
    const lunchStart   = (formData.get(`lunchStart-${d}`) as string | null)?.trim() || null;
    const lunchEnd     = (formData.get(`lunchEnd-${d}`)   as string | null)?.trim() || null;
    ...
    if (isOpen) {
      ...
      if (hasLunchBreak && lunchStart && lunchEnd) {
        if (lunchStart >= lunchEnd) {
          errors[`lunchRange-${d}`] = `${DAY_LABELS[d]}: 昼休み終了は開始より後にしてください`;
        } else if (lunchStart < openTime || lunchEnd > closeTime) {
          errors[`lunch-${d}`] = `${DAY_LABELS[d]}: 昼休みは営業時間内に設定してください`;
        }
      }
    }
    dayRows.push({ dayOfWeek: d, isOpen, openTime, closeTime, hasLunchBreak, lunchStart, lunchEnd });
  }
    ...
    ...dayRows.map((row) =>
      prisma.businessHour.upsert({
        ...
        update: {
          ...
          hasLunchBreak: row.hasLunchBreak,
